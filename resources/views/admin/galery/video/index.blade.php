...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center mb-1">
            ...
            <button type="button" data-bs-toggle="modal" data-bs-target="#TambahVideo" class="btn btn-outline-primary">
                <i class="icon-base ri ri-add-fill me-2"></i> Tambah
            </button>
        </div>
        @include('admin.galery.video.partials._modalAdd')
        <p class="mb-6">
            Daftar Galery Video
        </p>
        <!-- Role cards -->
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <p class="mb-0">Total {{ $galery->GaleryVideo->count() }} Video</p>
                                <ul class="list-unstyled d-flex align-items-center avatar-group mb-0">
                                    ...
                                    @endforeach
                                    @if ($galery->GaleryVideo->skip(3)->count() != 0)
                                        <li class="avatar">
                                            <span class="avatar-initial rounded-circle pull-up bg-lightest text-body"
                                                data-bs-toggle="tooltip" data-bs-placement="bottom"
                                                title="{{ $galery->GaleryVideo->skip(3)->count() }} more">{{ $galery->GaleryVideo->skip(3)->count() }}</span>
                                        </li>
                                    @endif
                                </ul>
                            </div>
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="role-heading">
                                    ...
                                    <a href="{{ route('galery-video.show', $galery->id) }}" data-bs-tooltip="tooltip"
                                        data-bs-placement="right" title="Lihat Video" class="role-edit-modal">
                                ...
                                <div class="dropdown ms-2">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li>
                                            <button class="dropdown-item" data-bs-toggle="modal"
                                                data-bs-target="#EditVideo{{ $loop->iteration }}">
                                    ...
                                    @include('admin.galery.video.partials._modalEdit')
                                </div>
                            </div>
                        </div>
                    </div>
