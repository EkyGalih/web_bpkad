...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            ...
            <form action="{{ route('pages-admin.store') }}" id="form" method="POST" enctype="multipart/form-data">
                <div class="card-body">
                    @csrf
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-floating form-floating-outline mb-6">
                                ...
                                <select name="jenis_link" id="jenis_link" class="select2 form-select"
                                    ...
                                    <option value="non-link" {{ old('jenis_link') == 'non-link' ? 'selected' : '' }}>Tanpa Link</option>
                                    <option value="link" {{ old('jenis_link') == 'link' ? 'selected' : '' }}>Link</option>
                                </select>
                            </div>
                            <div class="form-floating form-floating-outline mb-6">
                                <input type="{{ old('jenis_link') == 'link' ? 'text' : 'hidden' }}" name="link" class="form-control" id="link"
                                    placeholder="https://example.com/example or /example" value="{{ old('link') }}">
                                <label for="link" id="label-link" {{ old('jenis_link') == 'link' ? '' : 'hidden' }}>Ekternal
                                    Link</label>
                            </div>
                            <div class="form-floating form-floating-outline mb-6">
                        ...
                        <div class="card-footer">
                            <div class="d-flex justify-content-end gap-2">
                                <button class="btn btn-warning btn-lg" type="reset">
                                    <i class="icon-base ri ri-refresh-line icon-18px me-2"></i> Reset
                                </button>
