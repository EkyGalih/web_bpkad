...
@section('content_home')
    <section class="section-frame overflow-hidden">
        <div class="wrapper image-wrapper bg-image bg-overlay bg-overlay-300"
            data-image-src="{{ asset($posts->foto_berita) }}">
                        ...
                        <div class="post-header">
                            <div class="post-category text-line text-white">
                                <a href="#" class="text-reset"
                                    rel="category">{{ PostCategory($posts->posts_category_id) }}</a>
                            </div>
                            <!-- /.post-category -->
                            <h1 class="display-1 mb-4 text-white">{{ $posts->title }}</h1>
                            <ul class="post-meta text-white">
                                <li class="post-date"><i
                                        class="uil uil-calendar-alt"></i><span>{{ get_date($posts->created_at) }}</span>
                                </li>
                                <li class="post-author text-white"><span><i class="uil uil-user"></i><span>By
                                            {{ $posts->users->nama }}</span></span></li>
                                <li class="post-likes ms-auto">
                                    <i class="uil uil-eye"></i> {{ $posts->click }}
                                </li>
                            </ul>
                            <!-- /.post-meta -->
                        </div>
                                            ...
                                            <div>
                                                <ul class="list-unstyled tag-list mb-0">
                                                    @foreach (Tags($posts->tags) as $tags)
                                                        <li><a href="#"
                                                                class="btn btn-soft-ash btn-sm rounded-pill mb-0">{{ $tags }}</a>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                            </div>
                                                    ...
                                                    <article>
                                                        ...
                                                        <div class="post-header">
                                                            <div class="post-category text-line">
                                                                <a href="{{ route('post.show', [GetCategoryContent($post->posts_category_id), $post->slug]) }}"
                                                                    class="hover"
                                                                    rel="category">{{ PostCategory($post->posts_category_id) }}</a>
                                                            </div>
                                                            <!-- /.post-category -->
                                                            <h2 class="post-title h3 mt-1 mb-3"><a class="link-dark"
                                                                    href="{{ route('post.show', [GetCategoryContent($post->posts_category_id), $post->slug]) }}">{{ substr($post->title, 0, 50) }}...</a>
                                                            </h2>
                                                        </div>
                                                        <!-- /.post-header -->
                                                        <div class="post-footer">
                                                            <ul class="post-meta mb-0">
                                                                <li class="post-date"><i
                                                                        class="uil uil-calendar-alt"></i><span>{{ get_date($post->created_at) }}</span>
                                                                </li>
                    ...
                    <div class="widget">
                        <form action="{{ route('post.search') }}" class="search-form" method="POST">
                            ...
                            <div class="form-floating mb-0">
                                <input id="search-form" type="text" name="cari" class="form-control"
                                    placeholder="Search">
